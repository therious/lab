...
IPORT="${3:-3000}"  # port to publish on os (internal 3000 assumed) defaults to 3000
...
# run a docker build
docker build --no-cache --progress=plain -f "${DOCKERFILE}" -t "${IMAGE}" .  2>&1 | tee "${DOCKERFILE}.log"

# run the container
nohup docker run --pull=never -p "${PORT}:${IPORT}" --name="${CONTAINER}" "${IMAGE}" &
