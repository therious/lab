// Standalone implementation of oReduce (replaces @therious/utils dependency)
function oReduce<T, K extends string | number | symbol, V>(
  a: T[],
  f: (v: T, accum: Record<K, V>) => [K, V],
  o: Record<K, V> = {} as Record<K, V>
): Record<K, V> {
  return a.reduce((accum: Record<K, V>, v: T) => {
