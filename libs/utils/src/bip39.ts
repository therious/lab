const bip39WordList = [
  ...
  "raccoon", "radar", "radio", "rail", "rain", "raise", "rally", "ramp", "ranch", "random",
...
// given a hex hash string, and an optional character limit (use first n characters only)
// return a bip39 phrase that represents the hash, and can be reversed back to the hash
// (given the same hash length to reverse to)
...
function hashToWords(inputHash: string, limit?:number): string {
  ...
  while (tempNumber > 0) {
    const wordIndex = Number(tempNumber & BigInt(0x7FF)); // 0x7FF is 2047 in decimal, representing 11 bits
    words.push(bip39WordList[wordIndex]);
    tempNumber >>= 11n; // Shift right by 11 bits
  }
...
function wordsToHash(phrase: string, hashCharacters: number): string {
  ...
  for (let i = words.length - 1; i >= 0; i--) {
    const wordIndex = BigInt(bip39WordList.indexOf(words[i]));
    if (wordIndex === -1n) {
      throw new Error("invalid phrase exception");
    }
    number = (number << 11n) + wordIndex;
  }

  const requiredBits = hashCharacters * 4;
  const mask = (1n << BigInt(requiredBits)) - 1n;
  const hashNumber = number & mask;
  const hash = hashNumber.toString(16);

  // Ensure the hash is the correct length by padding with zeros if necessary
  const result = hash.padStart(hashCharacters, '0');
  const r = reverseString(result);
